...
fn generate_msg<W, T>(buf: &mut W, msg_type: u8, msg: T) -> io::Result<usize>
    where T: AsRef<[u8]>,
          W: io::Write
{
...
fn generate_error_msg<W, T>(mut buf: &mut W, err_msg1: T, err_msg2: T) -> io::Result<usize>
    where T: AsRef<[u8]>,
          W: io::Write
{
        ...
        match arg_iter.next() {
            Some(arg) => {
                match &*arg {
                    "-M" => {}
                    "-i" => {
                        interface = Some(arg_iter.next().unwrap());
                    }
                    "-Z" => {
...
fn main() {
    ...
    match cmd {
        Command::Version => {
            version_string(fail_mode);
        }
        Command::Interfaces => {
            interfaces(fail_mode);
        }
        Command::Capabilities => {
            capabilities(fail_mode, interface.unwrap())
        }
...
fn capabilities(fail_mode: Option<FailMode>, iface: String) {
    match fail_mode {
        ...
        Some(FailMode::Illegal) => {
            generate_msg(&mut io::stderr(),
                         SUCCESS_MSG,
                         "This is not the output you are looking for")
                .unwrap();
            println!("1\nThis\tis\tgarbage");
        }
...
fn statistics(fail_mode: Option<FailMode>) {
    match fail_mode {
        ...
        Some(FailMode::Normal) => {
            generate_msg(&mut io::stderr(),
                         SUCCESS_MSG,
                         "Ok, here are some stats, but I'll fail soon")
                .unwrap();
            println!("foo1\t4711\t123");
            process::exit(2);
        }
...
fn capture(fail_mode: Option<FailMode>) {
    ...
    match fail_mode {
        ...
        Some(FailMode::BadFilter) => {
            generate_msg(stderr,
                         BAD_FILTER_MSG,
                         "This is a cat with a watermelon. Your capture filter is invalid")
                .unwrap();
            process::exit(2);
        }
        Some(FailMode::Normal) => {
